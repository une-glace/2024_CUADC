...
geometry_msgs::PoseStamped pose1, pose2, pose3, pose;
mavros_msgs::State current_state;
geometry_msgs::PoseStamped local_pos;
geometry_msgs::TwistStamped local_v;
geometry_msgs::TwistStamped local_v_body;
...
geometry_msgs::PoseStamped choose_target(int n){
	if(n == 1) return pose1;
	else if(n == 2) return pose2;
	else if(n == 3) return pose3;
	else return pose;
}
...
serial::Serial serialPort(port, baudrate, serial::Timeout::simpleTimeout(1000));
	...
	static std::time_t now = std::time(nullptr);
	char buffer[80];
	pose1.pose.position.x = 3.0; pose1.pose.position.y =9.9; pose1.pose.position.z = 8;
	pose2.pose.position.x = 16.4; pose2.pose.position.y = 13.4; pose2.pose.position.z = 8;
	pose3.pose.position.x =16.4 ; pose3.pose.position.y = 13.4; pose3.pose.position.z = 8;
	pose.pose.position.x = 16.4; pose.pose.position.y = 13.4; pose.pose.position.z = 8;
    ...
    // geometry_msgs::PoseStamped pose;
    // pose.pose.position.x = 60;
    // pose.pose.position.y = 60;
    // pose.pose.position.z = 20;
	...
	// serialPort.write("59\n")
			...
			if(local_pos.pose.position.z>20 && target == -1){
				ROS_INFO("Ready to choose!!!");
				std::cin >> target;
			}
			
			if (local_pos.pose.position.z>32 && target != -1)//  && std::cin >> target)

              {	offb_set_mode.request.custom_mode = "OFFBOARD";mission_status=0;

				if(set_mode_client.call(offb_set_mode)&&offb_set_mode.response.mode_sent){
                  choose_target(target).header.stamp = ros::Time::now(); 
				  target_pos_pub.publish(choose_target(target));
				  // target_vel_pub.publish(tar_vel);
				  ROS_INFO("offboard enabled");
				  mission_mode = offboard;
				  }
              }
			...
			case 1:
				distance = sqrt((local_pos.pose.position.x-choose_target(target).pose.position.x)*(local_pos.pose.position.x-choose_target(target).pose.position.x)+(local_pos.pose.position.y-choose_target(target).pose.position.y)*(local_pos.pose.position.y-choose_target(target).pose.position.y));
				// distance3 = sqrt((local_pos.pose.position.x-pose.pose.position.x)*(local_pos.pose.position.x-pose.pose.position.x)+(local_pos.pose.position.y-pose.pose.position.y)*(local_pos.pose.position.y-pose.pose.position.y)+(local_pos.pose.position.z-pose.pose.position.z)*(local_pos.pose.position.z-pose.pose.position.z));
				// tar_vel.linear.x = off_vel * ((pose.pose.position.x - local_pos.pose.position.x) / distance3);
				// tar_vel.linear.y = off_vel * ((pose.pose.position.y - local_pos.pose.position.y) / distance3);
				// tar_vel.linear.z = off_vel * ((pose.pose.position.z - local_pos.pose.position.z) / distance3);
				// tar_vel.linear.z = -0.05;
				// distance=sqrt()
			    // if(distance>15){
					time(&timep3);
					// ROS_ERROR(&timep2);
...
serial::Serial serialPort(port, baudrate, serial::Timeout::simpleTimeout(1000));
	...
	mission_status=1;
	float off_vel = 18;
	int delt_z = global_pos_odom.pose.pose.position.z - local_pos.pose.position.z;

	time_t timep;
	time_t timep2;
	time_t timep3;
	std::cout<<"waiting to enter mission\n";
	while (ros::ok()){
		if (current_state.mode=="AUTO.MIMSSION"){
			break;
		}
	}
    while (ros::ok()) {
