...
int main(int argc, char** argv) {
	...
	pose.pose.position.x = 138.3;
	pose.pose.position.y = 145.4;
	...
	while (ros::ok()) {
		...
		case 0:{
			...
			if(cnt==2){
				...
				if(cnt==0){
				std::ifstream afile("/home/amov/Desktop/target.txt");
				...
				std::ifstream bfile("/home/amov/Desktop/target.txt");
				int intValue=0;
				float floatValue1, floatValue2;
			...
			if(local_pos.pose.position.z > 33 && cnt==1){
				...
				cnt=2;}
			if(local_pos.pose.position.z > 33 && cnt==0){
				ROS_INFO_STREAM_THROTTLE(1, "\033[1;32m \033[0m" << pose);
				offb_set_mode.request.custom_mode = "OFFBOARD";
				mission_mode = offboard;
				cnt=2;}
