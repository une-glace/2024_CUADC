...
inline void position_transfer_write(geometry_msgs::PoseStamped local_pos){
	...
	local_pos_transfer.pose.position.x=local_pos.pose.position.x*matrix[0][0]+local_pos.pose.position.y*matrix[0][1]+local_pos.pose.position.z*matrix[0][2];
	local_pos_transfer.pose.position.y=local_pos.pose.position.x*matrix[1][0]+local_pos.pose.position.y*matrix[1][1]+local_pos.pose.position.z*matrix[1][2];
	local_pos_transfer.pose.position.z=local_pos.pose.position.x*matrix[2][0]+local_pos.pose.position.y*matrix[2][1]+local_pos.pose.position.z*matrix[2][2];
...
int main(int argc, char **argv)
{
	...
	while (ros::ok())
	{
		...
		case 0:
			if (!current_state.armed && ok == 0)
			{
				...
				if (arming_client.call(arm_cmd) && arm_cmd.response.success)
				{
					...
					float x = origin_yaw[0] * (-1);
					float y = origin_yaw[1] * (-1);
					float z = origin_yaw[2] * (-1);
					float w = origin_yaw[3] * (-1);
		...
		case 1:
			...
			if (count == 1 && abs(local_pos.pose.position.x - target[0]) < 2 && abs(local_pos.pose.position.y - target[1]) < 2)
			{
