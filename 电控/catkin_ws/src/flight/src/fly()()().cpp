...
inline void position_transfer_write(geometry_msgs::PoseStamped local_pos) {
	...
	{
		outFile << local_pos_transfer.pose.position.x << " " << local_pos_transfer.pose.position.y << " " << local_pos_transfer.pose.position.z <<" "<<yaw<< std::endl;
...
int main(int argc, char** argv) {
	...
	float target[5][3] = { 0,200,30,  100,200,30,   0,300,30,   0,-100,25 };//目标航点坐标
	float origin_yaw[4] = {};
	...
	while (ros::ok()) {
		...
		case 1: {
			...
			if (count == 0 && (local_pos.pose.position.x - target[0][0]) < 5&& (local_pos.pose.position.y - target[0][1])<5) {
